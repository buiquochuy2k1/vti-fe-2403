...
const FullFilledCallBack = (data) => {
  console.log("[BÀI TẬP 2] xử lý bất đồng bộ thành công: ", data);
};
const ReJectCallBack = (err) => {
  console.log("[BÀI TẬP 2] Xử lý bất đồng bộ không thành công: ", err);
};
const SuccessAfterCallBack = () => {
  //   console.log("[BÀI TẬP 2] Chắc chắn khi xử lý bất đồng bộ thì ở đây sẽ chạy");
};
...
const Ex3Promise = new Promise((resolve, reject) => {
  const isSuccessFul = true; // ví dụ để test promise
  ...
  if (isSuccessFul) {
    ...
    reject("[BÀI TẬP 3] Thất bại"); // xử lý thất bại nhưng báo lỗi
  }
});

Ex3Promise.then((data) => {
  // xu ly khi nhan data
  console.log("[BÀI TẬP 3] Xử lý thành công: ", data);
})
  .catch((err) => {
    //xy ly khi xay ra loi
    console.log("[BÀI TẬP 3] Xử lý thất bại: ", err);
  })
  .finally(() => {});
